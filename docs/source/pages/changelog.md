...
-   Plotting keyword arguments for all plotting functions (`plot_force`, `plot_bmd`, `plot_sfd`, `plot_tmd`, `plot_def`): `figsize`, `ax` (existing matplotlib Axes), `scale`, `title`, `color`, `fill`, `alpha`, and `show`. Plotly equivalents are forwarded through convenience wrappers.
